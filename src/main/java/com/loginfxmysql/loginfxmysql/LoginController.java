...
public class LoginController implements Initializable {
    ...
    public void signIn(ActionEvent event) throws SQLException {
        ...
        if (username.isEmpty() && password.isEmpty()) {
            ...
            notification.hideAfter(new Duration(2000));
...
//            txt_username.setStyle("-fx-border-color: red;");
            ...
            DatabaseHandler jdbcDao = new DatabaseHandler();
            boolean flag = jdbcDao.validate(username, password);
            if (!flag) {
                Notifications notification = pushNotify("Login Error", "Username or password Incorrect");
                notification.hideAfter(new Duration(2000));
                notification.showError();
                txt_username.requestFocus();
            } else {
